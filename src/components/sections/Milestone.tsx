  ...
  return (
    <section className="relative w-full py-16 md:py-32 bg-white overflow-hidden">
      <div className="container mx-auto px-6">
        <div className="flex flex-col items-center text-center space-y-4 md:space-y-6 mb-12 md:mb-24 max-w-4xl mx-auto">
          <div className="w-8 md:w-12 h-1 bg-[#ef4444] rounded-full mx-auto" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-16 relative">
           ...
           {MILESTONES.map((mile, i) => (
             <div key={i} className="relative z-10 flex flex-col items-center text-center space-y-4 md:space-y-6 group">
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-[#122343] flex items-center justify-center text-white shadow-2xl transition-transform group-hover:scale-110">
                   <Icon icon={mile.icon} className="w-6 h-6 md:w-8 md:h-8" />
                </div>
                <div className="space-y-3 md:space-y-4">
                   <div className="flex flex-col items-center gap-1">
                      <span className="text-xl md:text-2xl font-black text-[#ef4444]">{mile.year}</span>
                      <h4 className="text-lg md:text-xl font-black text-[#122343] uppercase tracking-widest">{mile.title}</h4>
                   </div>
                   <p className="text-xs md:text-sm text-gray-400 leading-relaxed max-w-xs mx-auto">{mile.desc}</p>
                </div>
